...
"""
    algorithm68(ml0::Array{Float64,2}, ml1::Array{Float64,2}, ml2::Array{Float64,1})
...
"""                    
function algorithm68(ml0::Array{Float64,2}, ml1::Array{Float64,2}, ml2::Array{Float64,1})
    # cost: 6.6e+08 FLOPs
    ...
    # (Z18 W19 Z18^T) = tmp14
    stime7= time()
    ml6, ml4 = LAPACK.syev!('V', 'L', ml4)

    # tmp21: ml5, full, W19: ml6, diagonal_vector, Z18: ml4, full
    ml7 = Array{Float64}(undef, 230)
    # tmp268 = (Z18^T tmp21)
    stime8= time()
    gemv!('T', 1.0, ml4, ml5, 0.0, ml7)

    # W19: ml6, diagonal_vector, Z18: ml4, full, tmp268: ml7, full
    # tmp269 = (W19^-1 tmp268)
    ml7 ./= ml6
...
function write_algorithm68_to_eventlog(io, id, stamps)
...
'  ))
    write( io, string(id, ";", "trsv_1e+06;", "1e+06;", "tmp71 = (L2^-T tmp68);", "trsv!('L', 'T', 'N', ml1, ml2);", string(stamps[6]), ";", string(stamps[7]), '
'  ))
    write( io, string(id, ";", "gemv_4.6e+05;", "4.6e+05;", "tmp21 = (tmp56 tmp71);", "gemv!('N', 1.0, ml3, ml2, 0.0, ml5);", string(stamps[7]), ";", string(stamps[8]), '
'  ))
    write( io, string(id, ";", "ml6, ml4 = LAPACK.syev_4.06e+07;", "4.06e+07;", "(Z18 W19 Z18^T) = tmp14;", "ml6, ml4 = LAPACK.syev!('V', 'L', ml4);", string(stamps[8]), ";", string(stamps[9]), '
'  ))
    write( io, string(id, ";", "gemv_1.06e+05;", "1.06e+05;", "tmp268 = (Z18^T tmp21);", "gemv!('T', 1.0, ml4, ml5, 0.0, ml7);", string(stamps[9]), ";", string(stamps[10]), '
'  ))
    write( io, string(id, ";", "gemv_230;", "230;", "tmp269 = (W19^-1 tmp268);", "gemv!('N', 1.0, ml4, ml7, 0.0, ml8);", string(stamps[10]), ";", string(stamps[11]), '
'  ))
end